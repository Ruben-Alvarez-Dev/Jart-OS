...
```
Read: $JART_OS_HOME/documentation/JART-OS-CANONICAL-SPEC.md
```
...
```
Read: $JART_OS_HOME/TASKBOARD.md
```
...
echo "YOUR_NAME | $(date -u +%Y-%m-%dT%H:%M) | What you are doing" \
  > $JART_OS_HOME/.agent-lock/$(echo FILEPATH | tr / -).lock

# When done
rm $JART_OS_HOME/.agent-lock/$(echo FILEPATH | tr / -).lock
```
...
| File | Purpose |
|------|---------|
| JART-OS-CANONICAL-SPEC.md | THE law (1100+ lines) |
| STATUS-AND-PROGRESS.md | What works, what is missing |
| AGENT_PROTOCOL.md | This file — how to work here |
